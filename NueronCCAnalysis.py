...
def getFRInMeshPerNeuron(NeuronIDPerMesh,TotalTimeInMesh):
    conn=mysql.connector.connect(user='root', password ='AnuIfe2014$', database='NueroSCi')
    curr=conn.cursor()

    selectstmt = "  "
    selectstmt = selectstmt + " select X.mesh, (SELECT   count(*) +1   from NueroSCi.tblNeuronspikeI a,   "
    ...
    selectstmt = selectstmt + " and d.mesh=x.mesh    "
    selectstmt = selectstmt + " and a.NeuronID= " + str(NeuronIDPerMesh)  + "   "
    selectstmt = selectstmt + " )  SpikeCount   "
    selectstmt = selectstmt + "  FROM  "

    selectstmt = selectstmt + " (SELECT  distinct d.mesh as mesh from NueroSCi.tblLOCATIONDATAI d   order by d.mesh asc    "

    selectstmt = selectstmt + " )   X  "
    ...
    for row in data :
        varMesh = row[0]
        varMESHID.append(row[0])
        varNeuronSpikeCount.append(row[1])
        # 1000 is to convert the time into second from milliseconds

        x = 0.00
        if TotalTimeInMesh[varMesh]==0.00:
            x=0.00
        if TotalTimeInMesh[varMesh] > 0.00:
            x=float(row[1])/float(TotalTimeInMesh[varMesh])
    ...
    return NeuronIDPerMesh, varMESHID,varNeuronSpikeCount,varFiringRateOfNeuronsinMesh
pairnmesh =[]
corrcoef =[]
NumberOfNeurons=36
N=400
...
animalname ="I"
# purge the table
insertstmt ="DELETE from  NueroSCi.tblCorrCoefNeuron  where  AnimalName='"+animalname+"' "
commitstmt ="commit "
...
for  k in  range(0 , NumberOfNeurons ):
    meshIDk, varNueronIDk, varNeuronSpikeCountk, varFiringRateOfNeuronsinMeshk = getFRInMeshPerNeuron(k,TotalTimeInMesh)
    for j in  range(k+1 , NumberOfNeurons ):

        PairMesh = str(k) +":"+ str(j)
        meshIDj, varNueronIDj, varNeuronSpikeCountj, varFiringRateOfNeuronsinMeshj=getFRInMeshPerNeuron(j,TotalTimeInMesh)

        a, b = np.corrcoef(varFiringRateOfNeuronsinMeshk, varFiringRateOfNeuronsinMeshj)[0]


        insertstmt = (
            "INSERT INTO NueroSCi.tblCorrCoefNeuron(IDROW,IDCOL, CORRCOEF, AnimalName) VALUES('%s','%s', '%s', '%s')" % (
