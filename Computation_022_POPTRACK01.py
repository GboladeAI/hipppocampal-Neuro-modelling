...
def getSpikeCountInMesh( meshID,TotalTimeInMesh):
    ...
    ssql = "  SELECT   AnimalName, mesh,TimeInMesh,  "
    ...
    ssql = ssql + "+n_35"
    ssql = ssql + "+n_36"
    ssql = ssql + "      FROM NueroSCi.tblTrainNueronSpikeCount_ACtivity     "
    ssql = ssql + "      where mesh =      " + str(meshID) +"    "
    curr.execute(ssql)
    data = curr.fetchall()
    varNeuronSpikeCount = []
...
def ProcessMesh(x_axis,y_axis,myDt, testDataID):
    ...
    updatestmt = "  UPDATE NueroSCi.tblBPRM_Dt"+vmyDt+"00    "
...
arrYAXIS = []
selectstmt = "  "
selectstmt = selectstmt + " SELECT id,x_axis, y_axis FROM NueroSCi.tblBPRM_Dt"+vmyDt+"00 where  PredictedMesh is  null and PredictedMesh_BPRM_D is   null and mesh  between 14 and 20"
print selectstmt
curr.execute(selectstmt)
data = curr.fetchall()
