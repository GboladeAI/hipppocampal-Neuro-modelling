...
def ProcessMesh(x_axis,y_axis,myDt, testDataID):
    ...
    if np.sum(varNeuronSpikeCountTEST) != 0:
        for k in range(0,NumberOfMeshes-1):
            ...
            vaMeshID=k+1
            MyMesh.append(k+1)
            meshID, varNueronID, varNeuronSpikeCount ,TotalTimeINMesh = getSpikeCountInMesh_Train(vaMeshID,Dt)
            sumxx = 0.00
            ...
            for ll in range(0, sizeofarr ):
                SpikeCountforMesh = float(varNeuronSpikeCountTEST[ll] ) +  varNeuronSpikeCount[ll]
                summm =summm + SpikeCountforMesh
                #print str(float(varNeuronSpikeCountTEST[ll] ))  +  "      " + str(varNeuronSpikeCount[ll])
        ...
        plt.savefig("/Users/akingboladeshada/Desktop/Project_MSc/ProjectDATA_New/REPORT_DEPENDANT/Mesh" + str(outValmeshID) + "_" + str(
                testDataID) + "_prediction")
        plt.close()

        plt.bar(MyMesh, varProbInMeshTest)
        plt.suptitle("Dependant(PairewiseCorrelation): Test Mesh:" + str(outValmeshID) + " , Dt: " + str(
            float(myDt)) + "s  Predicted Prob.: " + str(
            maxprmesh) + " Mesh: " + str(prmesh))
        plt.xlabel("Mesh")
        plt.ylabel("Probabilty")

        plt.savefig("/Users/akingboladeshada/Desktop/Project_MSc/ProjectDATA_New/REPORT_DEPENDANT/Mesh" + str(
            outValmeshID) + "_" + str(
            testDataID) + "_prediction")
        plt.close()
...
selectstmt = selectstmt + " SELECT id,x_axis, y_axis FROM NueroSCi.tblBPRM_Dt100  where    PriorProb_mesh is not null and PredictedMesh_D is  null  "
