...
def ProcessMesh(x_axis,y_axis,myDt, testDataID):
    ...
    if np.sum(varNeuronSpikeCountTEST) != 0:
        ...
        plt.savefig(
            "/Users/akingboladeshada/Desktop/Project_MSc/ProjectDATA_New/REPORT_DEPENDANT_6/Mesh" + str(outValmeshID) + "_" + str(
                testDataID) + "_prediction")
        plt.close()



        updatestmt = "  UPDATE NueroSCi.tblBPRM_Dt600    "
...
selectstmt = selectstmt + " SELECT id,x_axis, y_axis FROM NueroSCi.tblBPRM_Dt600  where   PriorProb_mesh is not null  and PredictedMesh_D is  null  "
...
for i in range (0,k-1):
    ProcessMesh(arrXAXIS[i],arrYAXIS[i], 0.6, arrID[i])
