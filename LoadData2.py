    ...
    if ratCells.find('cells') != -1:
        ...
        for j in range(n):
            ...
            for i in range(o):
                ...
                brainregion = tspk[j][0][34][0]
                crossvalidation = 1  # 1 is for training data , 0 for test
                rndid=random.randint(0, 13000000)

                insertstmt = (
                    "INSERT INTO NueroSCi.tblBrainRegion(id,AnimalName, NeuronID, TimeSpike,BrainRegion,crossvalidation,rndid) VALUES('%s','%s', '%s', '%s','%s', '%s','%s')" % (
                        id, animalname, j + 1, timeatspike, brainregion,crossvalidation, rndid))
