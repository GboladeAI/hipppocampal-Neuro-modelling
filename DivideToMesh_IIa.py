...
plt.axis('off')
ax.scatter(xpoints,ypoints,MyData,color=varColor, s=.8)
#ax.title("Location of Rat (Ibsen) full maze of ("+str(X)+" X " +str(Y)+ ")")
p.show()


cur.execute("SELECT distinct d.id,d.AnimalName,d.time_at_location,d.x_axis,d.y_axis, d.mesh , a.CrossValidation from NueroSCi.tblNeuronspikeI a, NueroSci.tblMAP b,NueroSCi.tblLOCATIONDATAI d where a.id = b.id_tblNEURONSPIKE  and b.distance < 1 and b.id_tblLOCATIONDATA=d.id  and  d.x_axis!=0.00 and d.y_axis !=0.00    and a.TimeSpike >= 5285 and a.TimeSpike<=7353  AND a.CrossValidation=1     ")
# fetch all of the rows from the query
data = cur.fetchall ()
xpoints = []
ypoints = []
...
ax.scatter3D(xpoints,ypoints,MyTime,color='g')
ax.set_xlabel('X')
ax.set_ylabel('Y')
...
for k in range(int(Tot)):
  ...
  if countx == 0:
      ...
      ax.scatter(varxpoints, varypoints, color='g', s=.1)  # Or whatever you want in the subplot
...
plt.suptitle("Location of Rat (Ibsen) divided into smaller meshes  of ("+str(Tot)+ ")")
