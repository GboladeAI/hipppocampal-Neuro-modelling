...
def getSpikeCountInMesh_Test(TimeAtLocation, Dt,meshID):
    ...
    if np.sum(varNeuronSpikeCount) !=0:
        ...
        for nueronIDindex in range (0,length-1):
            if varNeuronSpikeCount[nueronIDindex]==1:
                ...
                x= nueronIDSpikeCounVal + nueronIDSpikeCounVal * float(vCorrCoef)
...
def ProcessMesh(x_axis,y_axis,myDt, testDataID):
    ...
    if np.sum(varNeuronSpikeCountTEST) != 0:
        ...
        for k in range(0, NumberOfMeshes - 1):
            ...
            vaMeshID = k + 1
            MyMesh.append(k + 1)
            meshID, varNueronID, varNeuronSpikeCount, TotalTimeINMesh, varNeuronFR = getSpikeCountInMesh_Train(vaMeshID,
                                                                                                      Dt)
            if TotalTimeINMesh >= 5.00:

                sizeofarr = len(varNueronID)
                print varNeuronSpikeCountTEST
                for l in range(0, sizeofarr - 1):

                    FIRINGRATE_train = varNeuronFR[l]
                    ...
                    Pneurons= ((FIRINGRATE_train * myDt) ** SpikeCount_Test) * exp(-1.00 * FIRINGRATE_train * myDt) \
                              / factorial(SpikeCount_Test)

                    varP = varP * Pneurons

                    #print "Pneurons=  " + str(Pneurons)
                    varProbNeurons.append(Pneurons)

                p = 1.00
                p = getDependentProb(varProbNeurons[0], varProbNeurons[1])
                for l in range(2, sizeofarr - 1):
                    # print"******************************************"
                    p = getDependentProb(p, varProbNeurons[l])
                    # print p
                ...
                PmeshD = priorProb * p
                PmeshI = priorProb * p
                varProbInMeshTest.append(PmeshD)
                # print  "Mesh ::" + str(k)   +"       Prior Probability::" + str(priorProb)  +"   Posterior Probability::"+ str(p)
                print  str(k) + "  ::" + str(priorProb) + "  ::" + str(PmeshI) + "  ::" + str(PmeshD)
        ...
        plt.savefig(
            "/Users/akingboladeshada/Desktop/Project_MSc/ProjectDATA_New/REPORT_DEPENDANT/Mesh" + str(outValmeshID) + "_" + str(
...
selectstmt = selectstmt + " SELECT id,x_axis, y_axis FROM NueroSCi.tblBPRM_Dt100  where mesh=7 "
