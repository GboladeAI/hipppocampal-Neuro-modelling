...
ax.plot3D(xpoints,ypoints,MyTime,color='g')
ax.set_xlabel('X')
ax.set_ylabel('Y')
...
cur.execute("SELECT d.id,d.AnimalName,d.time_at_location,d.x_axis,d.y_axis, d.mesh , a.CrossValidation from NueroSCi.tblNeuronspikeI a, NueroSci.tblMAP b,NueroSCi.tblLOCATIONDATAI d where a.id = b.id_tblNEURONSPIKE and  a.CrossValidation=0 and b.distance < 1 and b.id_tblLOCATIONDATA=d.id  and  d.x_axis!=0.00 and d.y_axis !=0.00    and a.TimeSpike >= 5285 and a.TimeSpike<=7353       ")
# fetch all of the rows from the query
data = cur.fetchall ()
xpoints = []
ypoints = []
...
# plot3D requires a 1D array for x, y, and z
# ravel() converts the 100x100 array into a 1x10000 array
ax.plot3D(xpoints,ypoints,MyTime,color ='r')
ax.set_xlabel('X')
ax.set_ylabel('Y')
ax.set_zlabel('Time @ location (s)')
